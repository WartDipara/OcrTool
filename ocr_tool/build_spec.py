...
# Collect CA bundle so SSL works in frozen exe
try:
    import certifi
...
# Bundle local model cache so exe doesn't need network on first run
MODEL_CACHE = Path.home() / ".paddlex"
if MODEL_CACHE.is_dir():
    add_data.append(f"{MODEL_CACHE}{os.pathsep}models")

cmd = "pyinstaller --onefile --windowed --clean --name OCRTool"
