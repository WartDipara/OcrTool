...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        self.setWindowTitle("OCR Tool GPU v1.0")
        ...
        title = QLabel("OCR Tool GPU")
        title.setObjectName("title")
        subtitle = QLabel("图片文字识别工具 (GPU 加速)")
        ...
        left_layout.addWidget(QLabel("文件列表"), 0)
        ...
        self._body_splitter.addWidget(left)
        self._body_splitter.addWidget(right)
        self._body_splitter.setStretchFactor(0, 35)
        self._body_splitter.setStretchFactor(1, 65)
    ...
    def _start_ocr(self):
        ...
        self._worker_thread.started.connect(lambda: self._worker.process(files, self._output_dir))
    ...
    def _on_file_error(self, file_path: str, error_msg: str):
        ...
        self._progress_panel.append_log(f"失败: {Path(file_path).name} — {error_msg}")
        self._update_button_states()
    ...
    def _on_all_done(self):
        ...
        if err:
            self._progress_panel.append_log(f"全部完成，成功 {len(done)} 张，失败 {len(err)} 张")
        else:
